...
/** POST { slug } → questions du diagnostic (sans indication de bonne réponse). */
export async function POST(req: NextRequest) {
  ...
  if (typeof slug !== "string" || !/^[a-z0-9-]{2,80}$/.test(slug))
    return jsonError("Parcours invalide.", 400);

  const context = await getDiagnosticContext(slug);
  if (!context) return jsonError("Parcours introuvable.", 404);
  ...
  try {
    ...
    return jsonError(err.message || "Le diagnostic a échoué.", status);
  }
}
