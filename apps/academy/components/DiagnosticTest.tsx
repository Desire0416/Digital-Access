...
import {
  ...
  Lightbulb, TrendingUp, RotateCcw, GaugeCircle,
} from "lucide-react";
import { cn, buttonClasses } from "@da/ui";
import type { DiagQuestion, DiagResult, DiagLevel, DiagVerdict } from "@/lib/diagnostic/types";
import { VERDICT_LABEL } from "@/lib/diagnostic/types";

/* Test de positionnement d'une formation, piloté par l'IA (§22.2) — carte
   d'appel + modale (intro → questions → résultat : niveau + verdict + conseils). */

type Phase = "intro" | "loading" | "questions" | "evaluating" | "result" | "error";

const LEVEL_RING: Record<DiagLevel, string> = {
  BEGINNER: "ring-brand-blue-vif/30",
  INTERMEDIATE: "ring-brand-violet/30",
  ADVANCED: "ring-accent/30",
};

/** Ton visuel du verdict (§22.2). */
const VERDICT_STYLE: Record<DiagVerdict, { border: string; bg: string; text: string; dot: string }> = {
  DIRECT_ACCESS: { border: "border-success/25", bg: "bg-success/[0.06]", text: "text-success", dot: "bg-success" },
  PREPARATORY: { border: "border-warning/30", bg: "bg-warning/[0.07]", text: "text-[#B45309]", dot: "bg-warning" },
  ORIENTATION: { border: "border-brand-blue-royal/25", bg: "bg-brand-blue-royal/[0.06]", text: "text-brand-blue-royal", dot: "bg-brand-blue-royal" },
  DISPENSATION: { border: "border-accent/25", bg: "bg-accent/[0.06]", text: "text-accent", dot: "bg-accent" },
};

export function DiagnosticTest({ slug, title }: { slug: string; title: string }) {
  ...
  // Jeton de run : incrémenté à chaque reset/close pour ignorer une réponse fetch
  // en vol qui reviendrait APRÈS fermeture (sinon elle écrase l'état réinitialisé).
  ...
  async function start() {
    ...
    try {
      ...
      if (my !== runId.current) return; // modale fermée entre-temps → on ignore
      if (!res.ok) throw new Error(data?.error || "Le test n'a pas pu démarrer.");
      const qs: DiagQuestion[] = Array.isArray(data.questions) ? data.questions : [];
      if (qs.length < 3) throw new Error("Test indisponible pour l'instant.");
      setQuestions(qs);
      setIdx(0);
      setPhase("questions");
    } catch (e) {
      ...
      <div className="relative overflow-hidden rounded-2xl border border-brand-violet/20 bg-gradient-to-br from-brand-violet/[0.06] to-brand-cyan/[0.06] p-5 sm:p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <h3 className="font-display text-base font-bold text-navy">Pas sûr que ce soit votre niveau ?</h3>
            <p className="mt-1 text-sm leading-relaxed text-text-secondary">
              Faites le <strong className="font-semibold text-navy">test de positionnement</strong> — 8 questions, ~2 min.
              L&apos;IA évalue votre niveau et vous dit si vous pouvez démarrer, où commencer, ou s&apos;il vaut mieux vous préparer.
            </p>
            <button
              type="button"
            ...
            >
              <Sparkles size={16} /> Faire le test
            </button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Test de positionnement"
          >
            <motion.div
            ...
            >
              ...
              <div className="flex items-center justify-between gap-3 border-b border-navy/[0.08] px-5 py-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className="font-display text-sm font-bold text-navy">Test de positionnement</p>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-6">
                ...
                {phase === "intro" && (
                  <div className="text-center">
                    <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-gradient-da text-white shadow-brand" aria-hidden>
                      <Sparkles size={26} />
                    </span>
                    <h4 className="mt-4 font-display text-lg font-bold text-navy">Évaluons votre niveau</h4>
                    <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-text-secondary">
                      8 questions adaptées à cette formation. À la fin, l&apos;IA vous indique votre niveau et un verdict :
                      accès direct, préparation conseillée, réorientation, ou dispense.
                    </p>
                ...
                {(phase === "loading" || phase === "evaluating") && (
                  <div className="flex flex-col items-center justify-center py-14 text-center">
                    ...
                    <p className="mt-5 font-display text-sm font-bold text-navy">
                      {phase === "loading" ? "Préparation de votre test…" : "Analyse de vos réponses…"}
                    </p>
                ...
                {/* RESULT */}
                {phase === "result" && result && <ResultView result={result} onClose={close} onRestart={reset} />}
...
function ResultView({ result, onClose, onRestart }: { result: DiagResult; onClose: () => void; onRestart: () => void }) {
  const vs = VERDICT_STYLE[result.verdict];
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      {/* Verdict */}
      <div className={cn("rounded-2xl border p-5 text-center", vs.border, vs.bg)}>
        <p className="text-xs font-bold uppercase tracking-wide text-text-muted">Verdict</p>
        <div className="mt-2 inline-flex items-center gap-2.5 rounded-full bg-surface-primary px-5 py-2 shadow-sm">
          <span className={cn("h-2.5 w-2.5 rounded-full ring-4", vs.dot, LEVEL_RING[result.recommendedLevel])} />
          <span className={cn("font-display text-base font-extrabold", vs.text)}>{VERDICT_LABEL[result.verdict]}</span>
        </div>
        <div className="mt-4 flex items-center justify-center gap-6 text-center">
          <div>
            <p className="font-display text-2xl font-extrabold text-navy">{result.score}%</p>
            <p className="text-[11px] text-text-muted">Maîtrise estimée</p>
          </div>
          <span className="h-8 w-px bg-navy/[0.1]" />
          <div>
            <p className="font-display text-sm font-bold text-navy">{result.levelLabel}</p>
            <p className="text-[11px] text-text-muted">Votre niveau</p>
          </div>
          <span className="h-8 w-px bg-navy/[0.1]" />
          <div>
      ...
      {result.summary && <p className="mt-4 text-sm leading-relaxed text-text-secondary">{result.summary}</p>}
      ...
      <div className="mt-5 flex flex-col gap-2.5">
        ...
        >
          <RotateCcw size={13} /> Refaire le test
        </button>
      </div>
    </motion.div>
  );
