...
export async function registerUser(input: RegisterInput): Promise<ActionResult> {
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: { name, email, password: hashed, roles: ["CLIENT", "LEARNER"] },
    ...
    const url = `${await webBaseUrl()}/auth/verify?token=${token}`;
    console.log("[auth] Lien de vérification:", url); // secours si l'email n'arrive pas
...
export async function resendVerification(email: string): Promise<ActionResult> {
  ...
  try {
    ...
    const url = `${await webBaseUrl()}/auth/verify?token=${token}`;
    console.log("[auth] Lien de vérification (renvoi):", url);
    await sendVerificationEmail(user.email, { name: user.name, url });
...
/* ─── Vérification du token (activation) — appelée au rendu de /auth/verify ─── */
...
export async function verifyEmailToken(
  ...
  try {
    ...
    console.error("[auth] verifyEmailToken:", err);
    return "invalid";
  }
}

/* ─── Connexion (Auth.js — session réelle) ─────────────────────────────────── */
...
export async function requestPasswordReset(email: string): Promise<ActionResult> {
  ...
  try {
    ...
    if (user) {
      ...
      const url = `${await webBaseUrl()}/auth/reset-password?token=${token}`;
      console.log("[auth] Lien de réinitialisation:", url);
